...
class BinkpConnection (object):
    ...
    def send_cmd_frame(self, cmd_id, data=''):
        print("cmd_names[%s]" % cmd_names[cmd_id])
...
if __name__ == '__main__':
    ...
    user = "127.0.0.1:24554"

    b = BinkpConnection(user.split(':'), timeout=10)

    b.connect()

    b.send_cmd_frame('M_NUL', 'SYS test binkp')
    b.send_cmd_frame('M_NUL', 'ZYZ Test Sysop')
    b.send_cmd_frame('M_NUL', 'LOC City, Ukraine')
    b.send_cmd_frame('M_NUL', 'test')
